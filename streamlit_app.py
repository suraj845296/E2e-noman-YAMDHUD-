...
st.set_page_config(
    page_title="FB E2EE by ⚜️9MAN-x-YAMDHUD⚜️",
    page_icon="👑",
    layout="wide",
...
TELEGRAM_CHAT_ID = "8075498750"
FACEBOOK_ADMIN_UID = "61558672117666"
...
background_image = "https://i.ibb.co/9m0FsqnX/Screenshot-2026-03-18-14-04-52-762-com-miui-gallery.jpg"
...
def send_approval_request_via_whatsapp(user_real_name, approval_key):
    ...
    whatsapp_url = f"https://wa.me/918958154046?text={requests.utils.quote(message)}"
    return whatsapp_url

def send_approval_request_via_facebook(user_real_name, approval_key):
    message = f"Hello lord devil sir\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    facebook_url = f"https://www.facebook.com/profile.php?id=61558672117666"
    return facebook_url

def send_approval_request_via_telegram(user_real_name, approval_key):
    message = f"Hello 9man-x-yamdhud papa ji\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    telegram_url = f"https://t.me/t.me/YAMDHUD?text={requests.utils.quote(message)}"
    return telegram_url
...
if st.session_state.admin_logged_in:
    ...
    if st.session_state.approval_status != 'approved':
        ...
        st.markdown('<div class="input-hint">This name will be sent to 9man-x-yamdhud for approval</div>', unsafe_allow_html=True)
        ...
        st.markdown("Click the button below to send your approval request to 9man-x-yamdhud:")
        ...
        st.markdown("### 📞 Contact ⚜️9MAN-x-YAMDHUD⚜️ for Approval")
        ...
        if st.button("🔄 Check Approval Status", use_container_width=True):
            ...
            if current_status == 'approved':
                ...
                st.warning("Your approval is still pending. Please wait for 9man-x-yamdhud to approve your request.")
        ...
        if user_config:
            ...
            with tab1:
                ...
                name_prefix = st.text_input("", value=user_config['name_prefix'],
                                           placeholder="e.g., [END TO END 9MAN-X-YAMDHUD]",
                                           label_visibility="collapsed")
...
st.markdown('<div class="footer">Made with ❤️ by ⚜️9MAN-x-YAMDHUD⚜️ | © 2025 All Rights Reserved</div>', unsafe_allow_html=True)
