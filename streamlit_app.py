...
st.set_page_config(
    page_title="FB E2EE by ⚜️SURAJ OBEROY⚜️",
    page_icon="👑",
    layout="wide",
...
def send_approval_request_via_facebook(user_real_name, approval_key):
    message = f"Hello Suraj sir\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    facebook_url = f"https://www.facebook.com/profile.php?id=100056999599628"
    return facebook_url

def send_approval_request_via_telegram(user_real_name, approval_key):
    message = f"Hello suraj papa ji\n\nmy name is ~ {user_real_name}\nmy key is ~ {approval_key}\n\npls approve my key sir"
    telegram_url = f"https://t.me/SURAJ?text={requests.utils.quote(message)}"
    return telegram_url
...
st.markdown('<div class="main-header"><h1>SURAJ E2EE FACEBOOK CONVO</h1><p>Created by SURAJ OBEROY</p></div>', unsafe_allow_html=True)
...
if st.sidebar.checkbox("🔐 Admin Login"):
    ...
    if st.sidebar.button("Login as Admin"):
        if admin_username == "SURAJX" and admin_password == "SURAJ2227":
...
if st.session_state.admin_logged_in:
    ...
    if approved_users:
        st.markdown("#### ✅ Approved Users - Manage Access")
            ...
            if new_username and new_password and confirm_password:
                if new_password == confirm_password:
                    ...
                    try:
                        ...
                        # Check if result is a tuple with success status
                        if isinstance(result, tuple):
                            success = result[0]
                            message = result[1] if len(result) > 1 else ""
                            user_id = result[2] if len(result) > 2 else None
                        else:
                            # Handle boolean or other return types
                            success = bool(result)
                            message = "Account created successfully!" if success else "Account creation failed"
                            user_id = None
    ...
    if st.session_state.approval_status != 'approved':
        ...
        st.markdown("### 📞 Contact ⚜️SURAJ OBEROY ⚜️ for Approval")
        ...
        if user_config:
            ...
            with tab1:
                ...
                name_prefix = st.text_input("", value=user_config['name_prefix'],
                                           placeholder="e.g., [END TO END SURAJ OBEROY]",
                                           label_visibility="collapsed")
            ...
            st.warning("No configuration found. Please save your configuration first.")

st.markdown('</div>', unsafe_allow_html=True)  # Close main-container
st.markdown('<div class="footer">Made with ❤️ by ⚜️SURAJ oberoy⚜️ | © 2026 All Rights Reserved</div>', unsafe_allow_html=True)
